...
Poskrbeti je pa potrebno še za ustrezno zajemanje teh metrik. To je zato, ker spremljanje metrike še ne pomeni njene ustreznosti. Članek \cite{se_metrics} to ilustrira s spremljanjem metrike ocene študenta. Za oceno tako izbere oddaljenost študenta od table, kjer bližji dobijo višjo oceno, kot tisti ki sedijo zadaj. Avtor s tem poudari, da čeprav spremljamo primerno metriko (oceno študenta), pa zajemanje te metrike ni ustrezno.

Metrike, ki pa so primerne in ustrezno zajete, razvijalcem omogočajo prepoznati \sn{ozko grlo} njihovih aplikacij, da jih lahko odpravijo ter tako omogočijo svojim uporabnikom boljšo izkušnjo.

V naslednjem poglavju opišem kaj sploh so metrike in kaj njihovo spremljanje. Spremljanje razdelim na dva načina in ju primerjam. Predstavim tudi orodja, ki to problematiko že rešujejo na tak ali drugačen način. V tretjem poglavju predstavim razlike v delovanju klasičnih spletnih strani in enostranskih spletnih aplikacij. Nadalje, predstavim pomembnost metrik v enostranskih spletnih aplikacijah in zakaj v takih aplikacijah pomembnejše kot v klasičnih. Nato se osredotočim na enostranske aplikacije in novosti, ki jih prinašajo in jih moramo upoštevati pri zajemanju metrik. Pri tem izpostavim razliko v delovanju navigacije. Dotaknem se še ene posebnosti enostranskih aplikacij -- komponent in zakaj metrik, ki spremljajo to posebnost ne smemo posplošiti. V četrtem poglavju predstavim razne metrike, ki jih je možno zajeti. Nato predstavim še metrike, ki sem jih zajel v moji rešitvi. Za vsako zajeto metriko prikažem tudi poročilo, ki ga mora rešitev generira ob zadostnem številu podatkov. V petem poglavju se osredotočim na zasnovo in implementacijo moje rešitve. Predstavim načrt platforme in razložim njeno delovanje. Nato opišem orodja, ki sem jih uporabil v implementaciji rešitve. Zaključim z demonstracijo uporabe implementirane storitve, kjer prikažem kako zajamemo vsako od spremljanih metrik in s povzetkom, kjer povzamem funkcionalnosti, ki jih moja rešitev implementira. Diplomsko delo zaključim s sklepnimi ugotovitvami in predlogi za nadaljne delo.
...
Obstaja več metrik, ki jih je smiselno spremljati. Nekatere od naštetih so specifične za enostranske aplikacije, medtem ko so druge uporabljane tudi v klasičnih spletnih straneh.
...
V nadaljevanju opišem s pomočjo katerih API-jev sem te metrike zajemal in podrobneje predstavim metrike, ki sem jih zajemal.

\section{Zajemanje metrik}

Za zajemanje metrik sem uporabil različne funkcionalnosti, ki jih brskalnik in Javascript ponujata.

Najpogosteje sem se poslužil objekta Date, s katerim sem zajel čas ob določenem trenutku. To sem uporabil za merjenje zagonskega časa aplikacije in časa nalaganja pogleda.

Pri merjenju zagonskega časa sem se poslužil uporabe HTML5 performance API-ja. Ta izpostavi metodo, ki nam vrne čas, ko je brskalnik pričel nalagati stran.
...
Izjemo sem naredil za eno lastnost performance API-ja in sicer PerformanceTiming objekt, ki nam izpostavi atribut navigationStart. Ta atribut je v opuščanju, vendar ga večina brskalnikov še vedno podpira. Kot nadomestilo tega atributa se uvaja atribut Performance objekta timeOrigin, ki pa ni še standardiziran. V svojem pristopu tako preverjam če je timeOrigin definiran in če ni, uporabim opuščeni PerformanceTiming objekt.

Uporabil sem še poslušalca dogodka (ang. event listener) premika miške. Ta posluša za spremembo položaja miške in ob spremembi zabeleži novo pozicijo miške.
...
V poročilu \ref{page_load_report} pričakovano dobimo zelo kratke čase nalaganja pogleda, saj je navigacija v enostranskih spletnih aplikacijah precej hitra.
...
Cilj diplomske naloge je bil zasnovati platformo za spremljanje metrik enostranskih spletnih aplikacij. Pri tem sem bil uspešen, saj mi je uspelo zasnovati in razviti platformo, ki metrike ustrezno spremlja in je neodvisna od uporabljenega ogrodja za gradnjo enostranskih spletnih aplikacij.

Za dosego tega, sem najprej raziskal kakšne metrike so sploh primerne za take aplikacije in v čem se razlikujejo od ostalih metrik. Iz teh metrik sem jih nato izbral nekaj, ki so se mi zdele najbolj reprezentativne za enostranske aplikacije in najbolje demonstrirajo razliko v spremljanju metrik.

Z implementacijo zasnovane platforme sem odkril tudi tehnične ovire pri spremljanju metrik. Pri tem sem se osredotočil na dve medsebojno povezani oviri -- veliko količino podatkov in dejstvo, da zbiranje metrik ne sme vplivati na uporabniško izkušnjo. Ti dve oviri sem obšel z uporabo praks za razvoj skalabilne programske opreme.
...
Za nadaljnje delo na zasnovani platformi, pa ni potrebno čakati na implementacijo novih API-jev. Veliko metrik, opisanih v poglavju \ref{ch2:sec5}, se ne zanaša na te nove API-je in lahko njihovo spremljanje precej hitro implementiramo. Smiselno je dodati tudi grafični vmesnik, ki nam te podatke prikaže v neki človeku bolj prijazni obliki. S tem lahko metrike približamo tudi nekomu, ki nima tehničnega znanja.

Da zaključim, dela je še veliko in ga razvijalcem, z interesom zbiranja metrik, ne bo zmanjkalo.
